...
class ProxyServerWithoutCache {
public:
    ProxyServerWithoutCache(int port) : server_port(port) {}
    void run();
...
void ProxyServerWithoutCache::run() {
    WSADATA wsaData;
    ...
    while (true) {
        ...
        if (new_socket != INVALID_SOCKET) {
            std::thread(&ProxyServerWithoutCache::handleClient, this, new_socket).detach();
        }
    }
